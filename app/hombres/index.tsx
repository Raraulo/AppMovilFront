...
import { addToCart, addToFavorites, getCart } from '../../utils/storage';
...
export default function HombresScreen() {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        const res = await fetch('http://192.168.1.5:8000/api/productos/');
        const data = await res.json();
        const productosHombres = data.filter((p: any) => p.genero === 'Hombre');
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {/* GIF decorativo */}
        <Image
          source={{ uri: 'https://media.tenor.com/Q8Bg7efpbR0AAAAi/rose.gif' }}
        ...
        {/* Título principal */}
        <Text style={[styles.title, { marginTop: 60 }]}>Parfums Pour Messieurs</Text>

        {/* Campo de búsqueda */}
        <TextInput
          placeholder="Recherche de Parfum..."
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            {selectedPerfume && (
              <>
                <Image
                  source={{ uri: selectedPerfume.url_imagen }}
