...
export default function FavoritosScreen() {
  ...
  const handleAddToCart = async () => {
    const perfume = favorites[selectedIndex];
    try {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (gestureState.dx < -swipeThreshold && selectedIndex < favorites.length - 1) {
          changeProduct('next');
          ...
          {favorites.length === 0 ? (
            <View style={styles.messageContainer}>
              <Text style={styles.largeMessage}>
                Aquí podrás guardar tus perfumes favoritos
              </Text>
            </View>
          ) : (
            <FlatList
              key="favorites-horizontal-list"
              data={favorites}
          ...
          {favorites[selectedIndex] && (
            ...
            >
              <View style={styles.modalImageBox}>
                <Image
                  source={{ uri: favorites[selectedIndex].url_imagen }}
                  style={styles.modalFullImage}
                />

                <TouchableOpacity
                  style={styles.heartButton}
                  onPress={() => handleToggleFavorite(favorites[selectedIndex].id)}
                  activeOpacity={0.9}
                >
              ...
              <View style={styles.modalTextBoxFull}>
                <View style={styles.namePriceRow}>
                  <Text style={styles.modalName}>
                    {favorites[selectedIndex].nombre}
                  </Text>
                  <Text style={styles.modalPrice}>
                    ${favorites[selectedIndex].precio}
                  </Text>
                </View>

                <Text style={styles.modalBrand}>
                  {favorites[selectedIndex].marca_nombre || "Maison Parfum"}
                </Text>

                <Text style={styles.modalLabel}>Descripción:</Text>
                <ScrollView>
                  <Text style={styles.modalDescription}>
                    {favorites[selectedIndex].descripcion ||
                ...
                <Text style={styles.modalInfo}>
                  Tipo:{" "}
                  {tipoEquivalencias[favorites[selectedIndex].tipo] ||
                    "Desconocido"}
                </Text>
